...
const char* fragment =
     ...
     "{"
       ...
       "if(tmat.x>=0)"
         "{"
           ...
           "incoming*=1+.1*(1-2*rough_map)*step(1,tmat.w);"
           "vec2 tval=max(texture(roughnessTexture,pos.zx*.61).zx,texture(roughnessTexture,pos.xz*.75).xz);"
           "tval=mix(tval,pow(texture(roughnessTexture,pos.zx*-3.43).xz,vec2(2))*.5,.25);"
           "incoming=mix(incoming*vec3(0,.5,1)*.2,incoming,mix(.75+.5*length(tval),1,step(-.001,pos.y)));"
           "vec3 emission=tmat.z*incoming;"
           "color+=absorption*emission;"
           "absorption*=incoming/pi;"
           "vec3 rv=reflect(rd,nor);"
           "float rough1=.3-.3*pow(min(tval.x,tval.y),.1),rough2=mix(.04,.02+.04*pow(1-rough_map,2),step(.01,pos.y));"
 ...
 "{"
   ...
   "vec3 ro=vec3(.6,.1,.7)*20,col=vec3(0),ww=normalize(vec3(.76,.38,.1)-ro),uu=normalize(cross(normalize(vec3(0,1,0)),ww)),vv=normalize(cross(ww,uu));"
   ...
   "float ffac=pow(smoothstep(.1,1.99,length(p)),4);"
   "const float depth_of_field=.1+.035*ffac,fov_length=40.4-.8*ffac,focus_distance=.45;"
   "vec3 lens_pos=ro+(lens_sample.x*uu+lens_sample.y*vv)*depth_of_field,vr=normalize(ro+(p.x*uu+p.y*vv+fov_length*ww)*focus_distance-lens_pos);"
   "gl_FragColor.xyz=min(render(lens_pos+17.3*vr,vr),15);"
 "}";
